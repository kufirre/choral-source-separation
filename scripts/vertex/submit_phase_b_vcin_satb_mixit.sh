...
RUN_ID="${RUN_ID:-phase-b-vcin-satb-mixit-$(date +%Y%m%d-%H%M%S)}"
...
DATASET_GCS_PATHS="${DATASET_GCS_PATHS:-processed/Dagstuhl_ChoirSet_satb,processed/ESMUC_Choir_satb}"
...
if [[ -z "${START_CHECKPOINT}" ]]; then
    echo "START_CHECKPOINT is required for Phase B (or set BOOTSTRAP_CKPT_URI)." >&2
    exit 1
fi
